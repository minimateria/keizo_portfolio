...
salon_list = pd.read_excel(r"取り込むファイル\salon_data.xlsx",dtype=str,na_filter=False)
...
#google検索で取得したページのソースの全テキストから引き当て
...
for index, row in tqdm(salon_list.iterrows(), total=len(salon_list)):
    if row['郵便番号'] == '':
        ...
        matched_row = master_list[master_list['住所1'].apply(lambda x: x in address) &\
                        master_list['住所2'].apply(lambda x: x in address) &\
                        master_list['住所3'].apply(lambda x: x in address)]

        # 引き当てられなかった住所をgoogle検索から取得
